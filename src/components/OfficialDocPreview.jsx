...
export default function OfficialDocPreview({ 
  ...
  // Dynamic Live Preview: Render actual .docx from Supabase Storage via Mammoth
  ...
  useEffect(() => {
    loadDynamicPreview();
  }, [template?.id, template?.code, template?.file_path, selectedCase?.id, JSON.stringify(formValues)]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {template.file_path && (
            ...
            >
              ...
              <span>{isRenderingHtml ? 'Memproses...' : 'Segarkan Pratinjau'}</span>
            </button>
          )}
      ...
      {/* Official Police Sheet Document Container */}
      <div className="police-doc-paper" id="printable-police-document">
        {isRenderingHtml ? (
          <div style={{ padding: '60px 20px', textAlign: 'center', color: '#1F2937' }}>
            <RefreshCw size={28} className="animate-spin" style={{ margin: '0 auto 12px', color: '#0284C7' }} />
            <div style={{ fontWeight: 700, fontSize: '14px' }}>Mengonversi File Asli .docx dari Supabase...</div>
            <div style={{ fontSize: '12px', color: '#6B7280', marginTop: '4px' }}>
              Memproses tata letak fisik dokumen, kop, tabel, dan variabel dinamis
            </div>
          </div>
        ) : renderedDocxHtml ? (
          /* Real physical .docx HTML rendered via Mammoth */
          <div 
            className="docx-mammoth-content"
            dangerouslySetInnerHTML={{ __html: renderedDocxHtml }}
          />
        ) : (
          /* Fallback structured format when template does not yet have a physical .docx in Supabase */
          <div>
            {!template.file_path && (
              <div className="no-print" style={{
                marginBottom: '20px',
                ...
                * Catatan: Template ini belum ditautkan dengan file fisik .docx di Supabase Storage. Menampilkan pratinjau tata letak standar Polri.
              </div>
            )}
